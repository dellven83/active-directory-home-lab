...
it](screenshots/03-setting-up-the-sysinternal-and-connecting-it-to-the-domain-user-account-i-created-for-it.png)

### Creating service account(1)

![Creating service
account(1)](screenshots/10-creating-service-account.png)

### installing window server 2025

![installing window server
2025](screenshots/14-installing-window-server-2025.png)

### Admin powershell (1)

![Admin powershell (1)](screenshots/15-admin-powershell.png)

### Creating an admin account

![Creating an admin
account](screenshots/22-creating-an-admin-account.png)

### Admin domain i made so i can have client join domain(1)

![Admin domain i made so i can have client join
domain(1)](screenshots/34-admin-domain-i-made-so-i-can-have-client-join-domain.png)

### setting up Domain server with foresr

![setting up Domain server with
foresr](screenshots/35-setting-up-domain-server-with-foresr.png)

## 🌐 Networking & DHCP

### Resrvation for printer

![Resrvation for printer](screenshots/01-resrvation-for-printer.png)

### dhcp to domain(1)

![dhcp to domain(1)](screenshots/05-dhcp-to-domain.png)
...
connection](screenshots/06-creating-client-to-connect-to-server-and-then-pinging-it-to-check-connection.png)

### dhcp p2

![dhcp p2](screenshots/12-dhcp-p2.png)

### dhcp day lease(1)

![dhcp day lease(1)](screenshots/17-dhcp-day-lease.png)

### DHCP printer reservation set up

![DHCP printer reservation set
up](screenshots/26-dhcp-printer-reservation-set-up.png)

### Configure client to same network as domain server

![Configure client to same network as domain
server](screenshots/27-configure-client-to-same-network-as-domain-server.png)

### dhcp finish(1)

![dhcp finish(1)](screenshots/28-dhcp-finish.png)

### dhcp setup reserve

![dhcp setup reserve](screenshots/29-dhcp-setup-reserve.png)
...
desktop](screenshots/02-mapping-shared-folder-to-client-desktop.png)

### Drive mapping for users

![Drive mapping for users](screenshots/16-drive-mapping-for-users.png)
...
restarting
computer](screenshots/19-making-a-network-mapp-so-client-shared-folder-remains-active-after-restarting-computer.png)

### quota for shared folder, so space is preserve

![quota for shared folder, so space is
preserve](screenshots/20-quota-for-shared-folder-so-space-is-preserve.png)

### Shared folder created from server

![Shared folder created from
server](screenshots/32-shared-folder-created-from-server.png)

### Setting up shared permissions for file sharing

![Setting up shared permissions for file
sharing](screenshots/33-setting-up-shared-permissions-for-file-sharing.png)
...
users](screenshots/04-putting-limitation-on-file-types-for-users.png)
...
service
account](screenshots/08-created-a-gpo-so-noone-else-could-logon-as-something-else-on-the-service-account.png)

### Desktop wallpaper group policy(1)

![Desktop wallpaper group
policy(1)](screenshots/09-desktop-wallpaper-group-policy.png)

### First GPO password policy(1)

![First GPO password
policy(1)](screenshots/11-first-gpo-password-policy.png)

### Account lockout policy to prevent brute force attack

![Account lockout policy to prevent brute force
attack](screenshots/13-account-lockout-policy-to-prevent-brute-force-attack.png)

### USB Storage deny GPO policy

![USB Storage deny GPO
policy](screenshots/21-usb-storage-deny-gpo-policy.png)

### Applying gpo to OUs

![Applying gpo to OUs](screenshots/24-applying-gpo-to-ous.png)

### Enabled restriction of Control panel for users

![Enabled restriction of Control panel for
users](screenshots/25-enabled-restriction-of-control-panel-for-users.png)
...
server](screenshots/31-me-longing-in-from-a-user-i-created-within-the-server.png)
...
restart(1)](screenshots/07-computer-starting-up-website-automatically-and-at-fulll-screen-after-restart.png)
...
restart](screenshots/18-making-prompt-for-window-to-automatically-startup-website-after-restart.png)

### Setting up a website as startup to see if it works

![Setting up a website as startup to see if it
works](screenshots/30-setting-up-a-website-as-startup-to-see-if-it-works.png)
...
155533](screenshots/23-screenshot-2026-07-03-155533.png)
